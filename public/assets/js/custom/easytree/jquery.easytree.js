    ...
    var EasyTree = function (jQueryContext, options) {
        ...
        function getNodesAsHtml(nodes, level, display) {
            ...
            for (i = 0; i < nodes.length; i++) {
                ...
                html += '<li>';
                //modified by abhinava dinesh to access data tags
                if(typeof(n.data.name) !== 'undefined'){
                html += '<span id="' + n.id + '" data-name="' + n.data.name + '" data-mime="' + n.data.mime + '" class="' + spanCss + ' ">'; // wrapper span
                } else {
        ...
        function convertHtmlToNode(element) {
            ...
            node.id = $el.attr('id');
            //Added by abhinava dinesh to access data attributes
            node.data = $el.data();
